...
CANDLE_DIR = os.path.join(TRADING_DIR, "candle_archive")
...
def fetch_intraday(sid, exch="NSE_EQ"):
    payload = {"securityId": str(sid), "exchangeSegment": exch, "instrument": "EQUITY", "interval": "5", "fromDate": today, "toDate": today}
    try:
        resp = requests.post("https://api.dhan.co/v2/charts/intraday", json=payload, headers=headers, timeout=10)
        if resp.status_code == 200: return resp.json()
    except Exception: pass
    return None

# ═══ BACKTEST-GRADE UNIVERSE (patched 2026-07-16) ═══
...
# 3) NSE GAINERS — durable Dhan-fallback function, called directly (patched 2026-07-16)
try:
