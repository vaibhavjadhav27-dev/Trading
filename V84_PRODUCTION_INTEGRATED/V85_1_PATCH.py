...
from execution_integrity import StopLossBuilder
...
def safe_place_hard_sl(gateway, security_id: str, qty: int, side: str,
    ...
    """Place a broker-side SL-M using StopLossBuilder (V11 — fixes DH-906)."""
    import logging
    _log = logging.getLogger("v85_1")
    side = side.upper()
    trigger = _round_price(trigger, tick)
    current_price = _positive(current_price)

    if current_price <= 0 or trigger <= 0:
        _log.error(f"HARD SL ABORT: sid={security_id} {side} trigger={trigger} px={current_price}")
        return None
    ...
    for attempt in range(2):
        try:
            sl_order = StopLossBuilder.build_sl_order(
                side=side,
                stop_level=float(trigger),
                current_ltp=float(current_price),
                qty=int(abs(qty)),
                security_id=str(security_id),
            )
            _log.info(
                f"HARD SL PLACING (attempt {attempt+1}): sid={security_id} {side} "
                f"trigger={sl_order['trigger_price']} price={sl_order['price']} "
                f"qty={sl_order['qty']} txn={sl_order['txn']} ltp={current_price}"
            )
            resp = gateway.place_order(
                security_id=str(security_id),
                qty=sl_order["qty"],
                price=sl_order["price"],
                transaction_type=sl_order["txn"],
            ...
            if resp and isinstance(resp, dict):
                ...
                if oid:
                    _log.info(f"HARD SL OK: sid={security_id} {side} sl_oid={oid}")
                    return {**resp, "triggerPrice": sl_order["trigger_price"], "verifiedStatus": "PENDING"}
        except ValueError as ve:
            _log.error(f"HARD SL DH906 PREVENTED (attempt {attempt+1}): sid={security_id} {side} — {ve}")
        except Exception as e:
            _log.error(f"HARD SL ERROR (attempt {attempt+1}): sid={security_id} {side} — {e}")

        if attempt == 0:
            import time; time.sleep(0.25)
            # Widen trigger for retry
            trigger = _round_price(trigger * (0.998 if side == "LONG" else 1.002), tick)

    _log.error(f"HARD SL FAILED after 2 attempts: sid={security_id} {side}")
    return None
...
def execute_entry_allocated(bot, candidate: dict, decision: dict, sizing: dict) -> Tuple[bool, str]:
    ...
    if fill.get("status") not in ("FILLED", "PARTIAL") or int(fill.get("qty", 0)) <= 0:
        ...
        time.sleep(0.5)
        _nq = int(bot.dhan.verify_position(sid, side))
        if (side == "LONG" and _nq > 0) or (side == "SHORT" and _nq < 0):
            log.warning(f"ENTRY_TIMEOUT RACE: {symbol} filled AFTER timeout! nq={_nq} — adopting")
            fill["qty"] = abs(_nq)
            fill["status"] = "FILLED"
            fill["price"] = float((bot.dhan.get_order_status(oid) or {}).get("averageTradedPrice", 0)) or px
        else:
