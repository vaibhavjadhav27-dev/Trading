...
from execution_integrity import ExecutionIntegrity, OrderState
...
class TradingBotV84(TradingBotV82):
    def __init__(self,dry_run=False):
        ...
        try:
            ...
            log.error(f'ExecutionIntegrity init failed: {_ei_e}')
            self.ei = None
                            ...
                            if _V853_LOADED:
                                ...
                                for _d, _c in ranked:
                                    try:
                                        ...
                                        # PATCH A: V8.5.5 early entries bypass timing gate
                                        _is_early = _d.get("setup_type","") in ("EARLY_ACCELERATION","V855_EARLY_ACCELERATION")
                                        _accept = (_decision.action == EntryAction.ENTER_NOW) or (_is_early and hasattr(EntryAction,"CONFIRMATION_PENDING") and _decision.action == EntryAction.CONFIRMATION_PENDING)
                                        if _accept:
                                            _v853_filtered.append((_d, _c))
                                            _pass_type = "EARLY_BYPASS" if (_is_early and _decision.action != EntryAction.ENTER_NOW) else "ENTER_NOW"
                                            log.info(f"V853 ENTER: {_c.get('symbol','?')}" + f" phase={_decision.phase.value} q={_decision.quality_score:.0f} t={_decision.timing_score:.0f} f={_decision.final_score:.0f} pass={_pass_type}")
                                        else:
                                        ...
                                        log.error(f"V853 REJECT ON ERROR: {_c.get('symbol','?')}: {_e}")
