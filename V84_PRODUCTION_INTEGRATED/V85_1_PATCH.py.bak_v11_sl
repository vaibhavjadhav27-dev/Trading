...
def safe_place_hard_sl(gateway, security_id: str, qty: int, side: str,
    ...
    if current_price <= 0:
        ...
        _lg.getLogger(__name__).error(f"HARD SL ABORT: current_price=0 for SID={security_id} — cannot place SL without valid price")
        return None
    ...
    if side == "LONG":
        ...
        transaction = "BUY"
        # FIX DH-906: BUY-side SL-M needs price > trigger (not 0.0)
        _sl_price = round(trigger * 1.05, 2)  # 5% above trigger for BUY SL-M
    else:
