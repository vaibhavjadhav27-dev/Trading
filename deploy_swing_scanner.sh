# BACKUP
cp swing_scanner.py swing_scanner.py.bak.$(date +%Y%m%d_%H%M%S)

# WRITE (quoted heredoc = no shell expansion)
cat > swing_scanner.py <<'PYEOF'
...
def run_swing_scan():
    ...
    return buy_signals[:15], watch_signals
...
# VERIFY
venv/bin/python3 -m py_compile swing_scanner.py && echo '✅ swing_scanner.py OK' || echo '❌ swing_scanner.py FAILED — restore .bak'
