#!/usr/bin/env python3
import warnings; warnings.filterwarnings("ignore", message=".*Task was destroyed.*")
"""
...
import orb_rescan  # Reform 2026-08-04
from patch_integrate import side_aware_entry, side_aware_monitor, side_aware_exit
from rolling_exit import RollingState, evaluate_rolling_exit
from trade_policy import confidence_pct, pick_side as policy_pick_side, MIN_CONVICTION
...
class DhanClient:
    ...
    def calculate_margin(self, security_id, qty, transaction_type, price):
        ...
        return self._request("POST", "/margincalculator", payload)
...
class DynamoClient:
    ...
    def pre_open_scan(self):
        ...
        self._pre_open_scores = {str(g.get("sid","")): g.get("rank_score",0) for g in gaps if g.get("sid")}
        log.info(f"Pre-open scan: {len(gaps)} stocks with significant gap")
        for g in gaps[:10]:
...
class TradingBot:
    ...
    def check_market_quality(self):
        ...
        if nifty_hist and isinstance(nifty_hist, dict):
            ...
            if len(closes) >= 50:
                ...
                if last_close < ema50:
                    log.info(f"Nifty {last_close:.0f} < EMA50 {ema50:.0f} — bearish context (trading remains enabled)")
        ...
        if not nifty_above:
            log.info("Nifty below VWAP — bearish/weak context; both LONG and SHORT remain eligible")
        # === REGIME DETECTION ===
        try:
    ...
    def select_candidates(self):
        ...
        hour = _now_dt.hour + _now_dt.minute / 60
        if _now_dt.weekday() >= 5 or hour < 9.25 or hour > 15.5:
        ...
        try:
            ...
            ltp_map = {}
            for _ws_try in range(3):  # WS enabled - 3 attempts with 20s retry
                _ltp_result = {}
        ...
        if ltp_map:
            ...
            def _quick_score(pair):
                st, px = pair
                sid = str(int(st["security_id"]))
                pc = self._prev_closes.get(sid, 0) or 0
                if pc <= 0: return 0.0
                move = abs((px-pc)/pc*100.0)
                return min(move/5.0,1.0)*70.0 + (30.0 if px > pc else 0.0)
            sorted_stocks = [s for s, _ in sorted(momentum_scored, key=_quick_score, reverse=True)]
        else:
        ...
        for stock in self.watchlist:
            ...
            gap_pct = (ltp - prev_close) / prev_close * 100
            # V8: direction-neutral candidate universe. Gap is a feature, not a side filter.
            # Only reject an extreme move when it is outside normal execution bounds; a
            # sudden mover can still qualify later if remaining opportunity is supported.
            if abs(gap_pct) >= float(getattr(config, 'GAP_REJECT', 15.0)):
                self.funnel_counts['3. Gap Filter']['rejected'] += 1
                self.rejected_stocks.append({
                    'ticker': stock['ticker'], 'ltp': ltp, 'gap_pct': gap_pct, 'rs': 0,
                    'failed_filter': 'EXTREME_GAP',
                    'detail': f'Absolute gap {abs(gap_pct):.2f}% >= {getattr(config, "GAP_REJECT", 15.0)}%',
                    'source': 'Watchlist'})
                continue
            self.funnel_counts['3. Gap Filter']['passed'] += 1
            # Signed stock-vs-NIFTY relative strength: positive = stronger, negative = weaker.
            rs = gap_pct - nifty_gap
            candidates.append({
                'ticker': stock['ticker'], 'security_id': stock['security_id'],
                'ltp': ltp, 'gap_pct': gap_pct, 'rs': rs, 'direction': 'UNDECIDED',
                'prev_close': prev_close, 'tier': 'CURATED'
            }),
        _pre = getattr(self, '_pre_open_scores', {})
        for _c in candidates:
            _sid = str(_c.get('security_id',''))
            if _sid in _pre and not _c.get('rank_score'):
                _c['rank_score'] = _pre[_sid]
        candidates.sort(key=lambda x: x['rs'], reverse=True)
        # Re-rank: only recompute if rank_score not already set by pre-open scan
        # (prevents overwriting the richer 87-pt score with the simpler 45-pt formula)
        for c in candidates:
            if not c.get("rank_score"):
        ...
        # V8: both directions compete for every eligible candidate.
        # Split by gap direction: positive-gap → LONG pool, negative-gap → SHORT pool
        _long_pool  = [c for c in candidates if c.get('gap_pct', c.get('gap', 0)) >= 0][:50]
        _short_pool = [c for c in candidates if c.get('gap_pct', c.get('gap', 0)) < 0][:50]
        # Candle fallback: if pool < 3 after filters, add momentum candle setups
        def _check_candles(sid, side):
            """Return True if 3+ consecutive 1-min candles confirm direction.
            Uses 1-min candles (not 5-min) so check works pre-ORB at 09:28 IST
            when only 2 five-min candles have closed but 13 one-min exist."""
            try:
                import time as _ct
                _ct.sleep(0.5)  # rate limit: max 2 candle calls/sec
                # Try 1-min first (more candles available pre-ORB)
                _oh = self.dhan.get_ohlc_intraday(str(int(sid)), 'NSE_EQ', '1')
                if not isinstance(_oh, dict):
                    _oh = self.dhan.get_ohlc_intraday(str(int(sid)), 'NSE_EQ', '5')
                if not isinstance(_oh, dict): return False
                cls = _oh.get('close', [])
                if len(cls) < 4: return False  # need at least 4 to check last 3
                c1, c2, c3 = float(cls[-3]), float(cls[-2]), float(cls[-1])
                if side == 'LONG':  return c3 > c2 > c1   # 3 green 1-min candles
                if side == 'SHORT': return c3 < c2 < c1   # 3 red 1-min candles
            except Exception: return False
        # Always fill LONG pool to 10 using 3-green-candle candidates
        _fallback_long = [c for c in candidates if c.get('direction','LONG') == 'LONG'
                          and c not in _long_pool][:15]
        for _fc in _fallback_long:
            if _check_candles(_fc['security_id'], 'LONG'):
                _fc['tier'] = 'CANDLE_MOMENTUM'
                _long_pool.append(_fc)
                log.info(f"CANDLE_MOMENTUM LONG: {_fc['ticker']} (3 green candles → top-10)")
        # Always fill SHORT pool to 10 using 3-red-candle candidates
        _fallback_short = [c for c in candidates if c.get('direction','LONG') == 'SHORT'
                           and c not in _short_pool][:15]
        for _fc in _fallback_short:
            if _check_candles(_fc['security_id'], 'SHORT'):
                _fc['tier'] = 'CANDLE_MOMENTUM'
                _short_pool.append(_fc)
                log.info(f"CANDLE_MOMENTUM SHORT: {_fc['ticker']} (3 red candles → top-10)")
        # GAP-FADE pool: positive-gap stocks showing 3 red 1-min candles
        # These are reviewed separately — direction confirmed at runtime
        _fade_pool = []
        for _fc in _long_pool:
            # Check: positive gap but 3 consecutive red candles
            if _check_candles(_fc["security_id"], "SHORT"):  # 3 red on a +gap stock
                _fc2 = dict(_fc)
                _fc2["tier"] = "GAP_FADE"
                _fc2["direction"] = "SHORT"  # reclassify for execution
                _fc2["_gap_fade"] = True
                _fade_pool.append(_fc2)
                log.info(f"GAP_FADE SHORT: {_fc2["ticker"]} gap={_fc2.get("gap_pct",0):+.2f}% (positive gap, 3 red candles → fade)")
                if len(_fade_pool) >= 5: break  # max 5 gap-fade candidates
        if _fade_pool:
            log.info(f"GAP_FADE pool: {[c["ticker"] for c in _fade_pool]}")
        self._fade_candidates = _fade_pool  # expose for email/analysis
        # Merge all pools: LONG first, then SHORT, then GAP-FADE
        # Each stock keeps its 'direction' tag so router routes correctly
        self.candidates = _long_pool + _short_pool + _fade_pool
        # Expose pools separately for email and scoring
        self._long_candidates  = _long_pool
        self._short_candidates = _short_pool + _fade_pool
        log.info(f"DUAL POOL: {len(_long_pool)} LONG | {len(_short_pool)} SHORT candidates")
            ...
            try:
                ...
                def _rvol_fn(c):
                    ...
                    try:
                        ...
                        # FIX 2026-08-10: _v[-1] is the incomplete forming candle (vol often 0).
                        # Sum today's candle volumes for true session volume-so-far.
                        _vol_now = float(sum(x for x in _v if x)) if _v else 0
                        _adv = filters_v2._m(c.get("ticker", "")).get("adv_20d", 0)
                        # FIX 2026-08-10: real minutes since 09:15 IST (matches summed session volume).
                        _now_ist = _dt.datetime.utcnow() + _dt.timedelta(hours=5, minutes=30)
                        _open_ist = _now_ist.replace(hour=9, minute=15, second=0, microsecond=0)
                        _mins = max(1, min(375, int((_now_ist - _open_ist).total_seconds() // 60)))
        ...
        # === DUAL-SCORE + CSV CALIBRATION LOGGER (added 2026-07-28) ===
        try:
            import csv as _csv, os as _os, datetime as _dt, time as _tt
            import indicators as _ind
            from dual_scorer import score_candidate_dual as _score_dual
            _nif_gap = self.nifty_data.get("gap", 0) if hasattr(self, "nifty_data") else 0.0
            _regime_now = getattr(self, "regime", getattr(self, "market_regime", "NORMAL"))
            _score_src = getattr(self, "_prefilter_candidates", None) or self.candidates
            _kept_sids_sc = {str(c.get("security_id", "")) for c in self.candidates}
            _csv_dir = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), "candle_archive")
            _os.makedirs(_csv_dir, exist_ok=True)
            _csv_path = _os.path.join(_csv_dir, f"candidate_scores_{_dt.date.today().isoformat()}.csv")
            _new_file = not _os.path.exists(_csv_path)
            with open(_csv_path, "a", newline="") as _cf:
                _w = _csv.writer(_cf)
                if _new_file:
                    _w.writerow(["scan_time","ticker","sid","ltp","gap_pct","rs","rvol",
                                 ...
                                 "body_L","body_S"])
                for _sc in _score_src:
                    _ssid = str(_sc.get("security_id", ""))
                    try:
                        _df = None
                        try:
                            _tt.sleep(0.3)
                            _oh = self.dhan.get_ohlc_intraday(str(int(_ssid)), "NSE_EQ", "5")
                            if isinstance(_oh, dict) and _oh.get("close"):
                                import pandas as _pd
                                _df = _pd.DataFrame({_k: [float(_x) for _x in _v]
                                    for _k, _v in _oh.items()
                                    if _k in ("open","high","low","close","volume")})
                        except Exception:
                            _df = None
                        _tick = _sc.get('ticker','')
                        _leading_names = [s[0] for s in getattr(self,'_leading_sectors',[])] if hasattr(self,'_leading_sectors') else []
                        _sec = _sc.get('sector','')
                        _sec_lead = any(_sec.upper() in str(s).upper() for s in _leading_names)
                        _sec_vs   = bool(_leading_names) and not _sec_lead
                        _ls, _ss, _brk = _score_dual(
                        ...
                        _brk = {"gap_L":0,"gap_S":0,"rs_L":0,"rs_S":0,"sector_L":0,"sector_S":0,
                                ...
                                "body_L":0,"body_S":0}
                        log.warning(f"DUAL_SCORE FAILED {_sc.get('ticker','?')}: {type(_se).__name__}: {_se}", exc_info=True)
                    _sc["long_score"] = _ls
                    _sc["short_score"] = _ss
                    _w.writerow([
            ...
            log.info(f"DUAL-SCORE CSV: wrote {len(_score_src)} rows -> {_csv_path}")
        except Exception as _dse:
            log.warning(f"Dual-score CSV logger failed (non-fatal): {_dse}")
        # === END DUAL-SCORE + CSV LOGGER ===
    ...
    def record_orb(self):
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = {executor.submit(fetch_orb, c): c for c in self.candidates[:100]}  # was [:8] - matched to WS path 2026-08-11
    ...
    def place_entry(self, candidate, ltp, score):
        ...
        if not balance or balance <= 0:
            ...
            return False
        self._last_balance = balance  # update for dynamic daily loss calc
        orb_rescan.persist_balance(self._last_balance)  # Reform R5
        ...
        # Confidence-tiered sizing unified with short path (user rule 2026-07-28)
        _long_score_180 = float(candidate.get("long_score") or 0)
        try:
            from short_live import size_position as _size_pos
            qty, _leverage, _deployed = _size_pos(available, ltp, score=_long_score_180)
        except Exception as _se:
            log.warning(f"size_position failed ({_se}) — fallback to calculate_safe_qty")
        ...
        adv = candidate.get("adv_shares", 0)
        if adv and adv > 0:
            qty = min(qty, int(adv * getattr(config, "ADV_MAX_PCT", 1.0) / 100.0))
        if candidate.get('tier') == 'DISCOVERY':
        ...
        if qty * ltp > available:
            ...
            if qty < config.MIN_QUANTITY:
                return False
        if qty < 1:
            log.warning(f"SIZING REJECTED: score={_long_score_180:.0f}/100 below 60% gate")
            return None
        # Place order
        ...
        r_value = sl_distance
        # PRE-ENTRY: read live Dhan positions to clear any stale active_trade
        try:
            _lp = self.dhan.get_positions()
            if isinstance(_lp, list):
                _live_sids = {str(p.get('securityId','')) for p in _lp if abs(int(p.get('netQty',0)))>0}
            else:
                _live_sids = set()
            if self.active_trade:
                _at_sid = str(self.active_trade.get('security_id',''))
                if _at_sid and _at_sid not in _live_sids:
                    log.warning(f"STALE active_trade cleared before entry: {self.active_trade.get('symbol','?')} not live at broker")
                    self.active_trade = None
                    self.dynamo.clear_active_trade()
        except Exception as _pev:
            log.debug(f'Pre-entry verify failed (non-blocking): {_pev}')
        if self.active_trade is not None:
            if not hasattr(self, 'active_trades_list'): self.active_trades_list = []
            self.active_trades_list.append(self.active_trade)
            log.info(f"MULTI-TRADE: {self.active_trade.get('symbol','?')} moved to overflow ({len(self.active_trades_list)})")
        self.active_trade = {
        ...
        # Place server-side hard SL (survives bot crash — long SL = SELL below entry)
        try:
            from short_live import place_hard_sl as _phs
            _sl_res, _sl_trig = _phs(self.dhan, candidate['security_id'], filled_qty, filled_price, "LONG")
            _sl_oid = _sl_res.get('orderId','') if isinstance(_sl_res, dict) else ''
            self.active_trade['sl_order_id'] = _sl_oid
            self.dynamo.save_active_trade(self.active_trade)
            log.info(f"🛡️ LONG HARD SL: ₹{_sl_trig:.2f} (0.75% below entry) orderId={_sl_oid}")
        except Exception as _slx:
            log.error(f"⚠️ LONG SL order FAILED — trade open but SL bot-loop only: {_slx}")
    ...
    def monitor_active_trade(self):
        ...
        if not self.active_trade:
            try:
                ...
                if isinstance(resp, list):
                    for pos in resp:
                        _nq = int(pos.get("netQty", 0))
                        if _nq != 0 and pos.get("productType") == "INTRADAY":
                            _is_short = _nq < 0
                            if _is_short:
                                _bpx = float(pos.get("sellAvg", 0) or pos.get("costPrice", 0) or 0)
                                _bsl = round(_bpx * 1.03, 2)   # short SL ABOVE entry (3% approx)
                                _brv = round(_bsl - _bpx, 2)
                            else:
                                _bpx = float(pos.get("buyAvg", 0) or 0)
                                _bsl = round(_bpx * 0.97, 2)   # long SL BELOW entry (3% approx)
        ...
        if getattr(config, 'ROLLING_EXIT_ENABLED', False):
            ...
            log.info(f"ROLLING_EXIT init: {_side} entry={entry_price:.2f}")
        ...
        # Rolling exit check
        if getattr(config, 'ROLLING_EXIT_ENABLED', False) and getattr(self, 'rolling_state', None):
            try:
                ...
                action, reason, exit_px = evaluate_rolling_exit(
                    self.rolling_state, current_ltp, _rsi, _vol_chg,
                    _red, _vwap_slope, _rs_entry, _hl, config
                )
                if action == 'ADVANCE':
                    log.info(f"ROLLING: {reason} | floor={self.rolling_state.floor_price:.2f}")
                elif action == 'EXIT':
                    log.info(f"ROLLING_EXIT: {reason} | px={exit_px:.2f}")
                    self.execute_exit(current_ltp, f"ROLLING_EXIT: {reason}")
                    return
            except Exception as e:
                log.warning(f"Rolling exit error: {e}")
        ...
        # Dead-trade exit REMOVED — Rolling Exit + SMART_EXIT_v2 + SL handle all exits
        ...
        if not getattr(config,'ROLLING_EXIT_ENABLED',False) and r_multiple >= config.TRAIL_PHASE3_TRIGGER:
            ...
            # TRENDING_UP: skip T1 lock — hold to T2 for bigger move
            _reg = getattr(self, 'regime', 'NORMAL')
            if _reg in ('TRENDING_UP', 'FULL'):
                phase = 'PHASE1_TRENDING_HOLD'
                new_sl = trailing_sl  # don't advance — wait for T2
                log.info(f"TRENDING_UP: T1 skip — holding to T2 (+{config.TRAIL_PHASE2_TRIGGER}R)")
            else:
                phase = 'PHASE1'
            ...
            if candles_5m and isinstance(candles_5m, dict):
                ...
                if len(closes) >= 3:
                    ...
                    if len(volumes) >= 5 and sum(volumes[-5:]) > 0:
                        ...
                        if self.active_trade: self.active_trade['last_vwap'] = vwap_approx
        ...
        elif now_ist() >= ist_time(15, 9):
            exit_reason = "MANDATORY_EXIT_3:09PM"

        if exit_reason:
            self.exit_trade(current_ltp, exit_reason)
            # Also close all overflow trades at mandatory exit
            for _ot in list(getattr(self, 'active_trades_list', [])):
                try:
                    _ot_sid = _ot.get('security_id','')
                    _ot_ld = self.fetch_ltp_concurrent([str(_ot_sid)])
                    _ot_ltp = _ot_ld.get(str(_ot_sid), 0)
                    if _ot_ltp > 0:
                        _prev = self.active_trade
                        self.active_trade = _ot
                        side_aware_exit(self, _ot_ltp, exit_reason)
                        self.active_trade = _prev
                        log.info(f'EOD overflow exit: {_ot.get("symbol","?")} @ {_ot_ltp}')
                except Exception as _oe:
                    log.warning(f'Overflow EOD exit failed: {_oe}')
            self.active_trades_list = []
            return
    ...
    def exit_trade(self, exit_price, reason):
        ...
        # MULTI-TRADE: promote next overflow trade when this one closes
        _ov = getattr(self, 'active_trades_list', [])
        if _ov:
            self.active_trade = _ov.pop(0)
            log.info(f"MULTI-TRADE: promoted {self.active_trade.get('symbol','?')} from overflow ({len(_ov)} remaining)")
        else:
            self.active_trade = None
        self.dynamo.clear_active_trade()
    ...
    def scan_for_breakout(self):
        ...
        M = getattr(config, "MEASURED_MOVE_M_TRENDING", 1.5) if regime in ("TRENDING_UP", "TRENDING_DOWN") else getattr(config, "MEASURED_MOVE_M_NORMAL", 1.25)
        ...
        for candidate in self.candidates:
            ...
            if ltp <= 0:
                continue
            if float(candidate.get("long_score", 0) or 0) < 50.0:  # CANDIDATE_MIN (v8_rescore adds +25 at entry)
                continue
            orb = self.orb_data.get(sid, {})
            ...
            # V8 sudden-move support: if price has already cleared the measured target,
            # do not reject automatically. Require high conviction, RVOL and at least +0.40%
            # of remaining supported opportunity from the live entry.
            if target <= buffered_entry:
                rvol = float(candidate.get("rvol", 0) or 0)
                if (float(candidate.get("long_score", 0) or 0) < 70.0 or
                    rvol < float(getattr(config, "SUDDEN_MOVE_RVOL_MIN", 1.5))):
                    continue
                target = buffered_entry * (1.0 + float(getattr(config, "SUDDEN_MOVE_MIN_EXPECTED_MOVE_PCT", 0.40))/100.0)
            expected_r = (target - buffered_entry) / risk_per_share
            expected_move_pct = abs(target-buffered_entry)/buffered_entry*100.0 if buffered_entry>0 else 0.0
            if expected_move_pct < float(getattr(config, "MIN_EXPECTED_MOVE_PCT",0.40)):
                continue
            if expected_r < MIN_R:
                continue
            ...
            expected_move_pct = abs(target - buffered_entry) / buffered_entry * 100.0 if buffered_entry > 0 else 0.0
            candidate["_expected_target"] = target
            candidate["_expected_move_pct"] = expected_move_pct
            candidate["_planned_entry"] = buffered_entry
            candidate["_planned_sl"] = sl
        ...
        # Return ALL qualified breakouts (multi-trade: each enters independently)
        log.info(f"{len(survivors)} qualified breakouts | regime={regime} M={M} | "
                 f"top={survivors[0]['candidate'].get('ticker','?')} "
                 f"R={survivors[0]['expected_r']:.2f}")
        for _sv in survivors:
            _sv["candidate"]["_planned_entry"] = _sv["entry"]
            _sv["candidate"]["_planned_sl"]   = _sv["sl"]
        # Return best as primary (router handles multi-entry via active_trades_list)
        best = survivors[0]
        return (best["candidate"], best["entry"], best["expected_r"])

    def scan_for_breakdown(self):
        """DOWNSIDE mirror of scan_for_breakout (SHORT side).
        Entry BELOW orb_low, SL ABOVE entry, target BELOW. Fail-safe: never raises.
        Returns (candidate, entry_price, expected_r) or None.
        NOTE: additive — not wired into the run loop until Stage 3."""
        try:
            from dhan_charges import dhan_charges_mis
        except Exception:
            dhan_charges_mis = lambda q, e, s=None: 25.0
        ...
        if not self.candidates or not self.orb_data:
            return None
        # FIX 2026-08-04: only scan SHORT-pool stocks
        # LONG candidates must never be shorted via breakdown
        _short_pool = getattr(self, '_short_candidates', None) or self.candidates
        sids = [str(c['security_id']) for c in _short_pool
                if c['security_id'] in self.orb_data]
        if not sids:
            return None
        ltp_data = self.fetch_ltp_concurrent(sids)
        ...
        rank_of = {c['security_id']: i for i, c in enumerate(_short_pool)}
        BASE, STEP, CAP, MIN_R, COST_RATIO = 0.25, 0.05, 0.60, 1.5, 3.0
        survivors = []

        for candidate in _short_pool:
            if candidate.get("security_id") in getattr(config, "LONG_ONLY_SIDS", set()):
                continue  # Non-F&O stock — LONG only, skip breakdown scan
            sid = candidate["security_id"]
            if sid not in self.orb_data:
                continue
            ltp = ltp_data.get(str(sid), 0)
            if ltp <= 0:
                continue
            if float(candidate.get("short_score", 0) or 0) < 50.0:  # CANDIDATE_MIN (v8_rescore adds +25 at entry)
                continue
            orb = self.orb_data.get(sid, {})
            orb_high = orb.get("high", 0)
            orb_low = orb.get("low", 0)
            orb_range = orb_high - orb_low
            if orb_range <= 0 or orb_low <= 0:
                continue
            if (orb_range / orb_low) * 100 < config.ORB_MIN_RANGE_PCT:
                continue

            ri = rank_of.get(sid, len(self.candidates))
            rank_frac = min(BASE + ri * STEP, CAP)
            buffered_entry = orb_low - rank_frac * orb_range   # entry BELOW orb_low
            if ltp > buffered_entry:                            # not broken down yet
                continue

            sl = orb_low + 0.3 * orb_range                      # SL ABOVE entry
            sl = max(sl, ltp * 1.005) if ltp > 0 else sl        # safety: SL above entry
            risk_per_share = sl - buffered_entry
            if risk_per_share <= 0:
                continue
            target = orb_low - M * orb_range                    # target BELOW
            if target >= buffered_entry:
                rvol = float(candidate.get("rvol", 0) or 0)
                if (float(candidate.get("short_score", 0) or 0) < 70.0 or
                    rvol < float(getattr(config, "SUDDEN_MOVE_RVOL_MIN", 1.5))):
                    continue
                target = buffered_entry * (1.0 - float(getattr(config, "SUDDEN_MOVE_MIN_EXPECTED_MOVE_PCT", 0.40))/100.0)
            expected_r = (buffered_entry - target) / risk_per_share
            expected_move_pct = abs(buffered_entry-target)/buffered_entry*100.0 if buffered_entry>0 else 0.0
            if expected_move_pct < float(getattr(config, "MIN_EXPECTED_MOVE_PCT",0.40)):
                continue
            if expected_r < MIN_R:
                continue
            ...
            if qty < 1:
                continue
            target_gross = qty * (buffered_entry - target)
            est = dhan_charges_mis(qty, buffered_entry, target)
            if target_gross < COST_RATIO * est:
                continue
        ...
        survivors.sort(key=lambda d: (-d["expected_r"], d["rank_index"]))
        best = survivors[0]
        log.info(f"{len(survivors)} qualified BREAKDOWNS | regime={regime} M={M} | "
    ...
    def _sector_snapshot(self):
        """Read all configured NIFTY sectors and return scored leader/laggard lists."""
        try:
        ...
        prev = {}
        try:
            with open("sector_prev_close.json") as f:
                prev = json.load(f)
        except Exception:
            pass
        out = {}
        def fetch(item):
            name, sid = item
            try:
                d = self.dhan.get_ohlc_intraday(str(sid), "IDX_I", "5")
                closes = d.get("close", []) if isinstance(d, dict) else []
                return name, float(closes[-1]) if closes else 0.0
            except Exception:
                return name, 0.0
        items = [(n, sid) for n, sid in SECTOR_INDICES.items() if sid and not str(sid).startswith("REPLACE")]
        with ThreadPoolExecutor(max_workers=4) as ex:
            for name, ltp in ex.map(fetch, items):
                if ltp > 0:
                    out[name] = ltp
        nifty_ltp = float(self.nifty_data.get("ltp", 0) or 0)
        nifty_prev = float(self.nifty_data.get("prev_close", 0) or 0)
        nifty_ret = ((nifty_ltp - nifty_prev) / nifty_prev * 100.0) if nifty_prev > 0 else 0.0
        rows = []
        for name, ltp in out.items():
            pc = float(prev.get(name, 0) or 0)
            if pc <= 0:
                continue
            ret = (ltp - pc) / pc * 100.0
            rows.append({"sector": name, "ltp": ltp, "return_pct": ret, "rs_vs_nifty": ret - nifty_ret})
        rows.sort(key=lambda x: x["rs_vs_nifty"], reverse=True)
        leading = [(r["sector"], r["return_pct"], r["rs_vs_nifty"]) for r in rows if r["return_pct"] > 0 and r["rs_vs_nifty"] > 0.5][:3]
        lagging = [(r["sector"], r["return_pct"], r["rs_vs_nifty"]) for r in reversed(rows) if r["return_pct"] < 0 and r["rs_vs_nifty"] < -0.5][:3]
        return rows, leading, lagging

    def maybe_record_market_snapshot(self, force=False):
        """Persist NIFTY + sector context every 30 minutes and update live context."""
        now = now_ist()
        if now.weekday() >= 5:
            return False
        start = ist_time(9, 30); end = ist_time(15, 0)
        if not force and not (start <= now <= end):
            return False
        slot = now.strftime("%Y-%m-%d") + f"-{(now.hour*60 + now.minute)//30:03d}"
        if not force and getattr(self, "_last_market_snapshot_slot", None) == slot:
            return False
        # Refresh NIFTY/market regime first; this is a READ only.
        try:
            self.check_market_quality()
        except Exception as exc:
            log.warning(f"Market snapshot NIFTY read failed: {exc}")
        rows, leading, lagging = self._sector_snapshot()
        self._leading_sectors = leading
        self._lagging_sectors = lagging
        self.nifty_data["sector_snapshot"] = rows
        self.nifty_data["market_snapshot_ts"] = now.isoformat()
        path_dir = "candle_archive"
        os.makedirs(path_dir, exist_ok=True)
        path = os.path.join(path_dir, f"market_snapshot_{now.strftime('%Y-%m-%d')}.csv")
        import csv as _csv
        fields = ["ts_ist", "regime", "nifty_ltp", "prev_close", "gap_pct", "vwap", "ema20", "ema50", "above_vwap", "above_ema50", "leading_sectors", "lagging_sectors", "sector_rows"]
        row = {
            "ts_ist": now.strftime("%Y-%m-%d %H:%M:%S"),
            "regime": getattr(self, "regime", "NORMAL"),
            "nifty_ltp": self.nifty_data.get("ltp", ""),
            "prev_close": self.nifty_data.get("prev_close", ""),
            "gap_pct": self.nifty_data.get("gap", ""),
            "vwap": self.nifty_data.get("vwap", ""),
            "ema20": self.nifty_data.get("ema20", ""),
            "ema50": self.nifty_data.get("ema50", ""),
            "above_vwap": float(self.nifty_data.get("ltp", 0) or 0) > float(self.nifty_data.get("vwap", 0) or 0),
            "above_ema50": float(self.nifty_data.get("ltp", 0) or 0) > float(self.nifty_data.get("ema50", 0) or 0),
            "leading_sectors": json.dumps(leading),
            "lagging_sectors": json.dumps(lagging),
            "sector_rows": json.dumps(rows),
        }
        ...
        with open(path, "a", newline="") as fh:
            ...
            w.writerow(row)
        self._last_market_snapshot_slot = slot
        log.info(f"MARKET SNAPSHOT 30m: regime={self.regime} NIFTY={self.nifty_data.get('ltp')} gap={self.nifty_data.get('gap')}% leaders={[x[0] for x in leading]} laggards={[x[0] for x in lagging]}")
        return True
        ...
        try:
            ...
            if active and 'symbol' in active:
                ...
                try:
                    ...
                    _live = 0
                _expected_side = str(active.get('side', 'LONG')).upper()
                _sign_ok = (_live > 0) if _expected_side == 'LONG' else (_live < 0)
                if _live != 0 and _sign_ok:
                    active['qty'] = str(abs(_live))
                    self.active_trade = active
                    log.info(f"Recovered active trade: {active['symbol']} side={_expected_side} (broker netQty={_live})")
                else:
            ...
            if self.candidates:
                candidate_sids = [str(c["security_id"]) for c in self.candidates[:100]]
            ...
            if regime in ("CONSERVATIVE","NORMAL","FULL","TRENDING_UP","TRENDING_DOWN") and getattr(config, "BEARISH_LIVE_ENABLED", True):
                try:
                    ...
                    log.info(f"BEARISH day. Sector rotation = BONUS ONLY (not hard filter)")  # FIXED: was hard gate
                    if not sector_prev:
                        ...
                        self._leading_sectors = leading or []
                        log.info(f"  Leading sectors: {leading or []}")
                        if not leading:
                            log.info("  No leading sector found — all candidates stay as LONG (sector=bonus only)")
                        else:
                            for c in self.candidates:
                                ...
                                gap = c.get("gap_pct", 0)
                                if ok:
                                    # Sector IS leading for this stock — bonus
                                    c["_sector_bonus"] = 5
                                    log.info(f"    {c.get('ticker','?')}: {reason} (+5 bonus)")
                                else:
                                    # Sector NOT leading — small penalty, but NEVER reclassify direction
                                    # Positive-gap stocks MUST stay LONG regardless of sector
                                    if gap > 0:
                            # sector not leading: no score penalty (kept LONG, ordering only)
                                        log.info(f"    {c.get('ticker','?')}: {reason} (-5 penalty, stays LONG)")
                                    else:
                                        # Negative-gap stock with weak sector = stronger short
                                        c["short_score"] = min(60, c.get("short_score", 30) + 5)
                                        log.info(f"    {c.get('ticker','?')}: {reason} (+5 short bonus)")
                            log.info(f"  Long candidates: {[c.get('ticker','?') for c in self.candidates if c.get('gap_pct',0) > 0]}")
                            log.info(f"  Short candidates: {[c.get('ticker','?') for c in self.candidates if c.get('gap_pct',0) <= 0]}")
                except Exception as e:
            ...
            try:
                ...
                shortlist_data = collect_shortlist_data(
                    ...
                    self.orb_data,
                    {**self.nifty_data,
                     'regime': getattr(self, 'regime', getattr(self, 'market_regime', 'UNKNOWN')),
                     'banknifty': self.nifty_data.get('banknifty', 'N/A')},
                    self.funnel_counts
                )
            ...
            while now_ist() < ist_time(15, 9):
                ...
                try:
                    self.maybe_record_market_snapshot()
                except Exception as _msx:
                    log.warning(f"30-min market snapshot failed: {_msx}")
                if time.time() > breakout_deadline and not self.active_trade:
                    log.warning("Breakout scan timeout (30 min) - no breakout found")
                    break

                # Monitor active trade (runs IN PARALLEL with entry scan)
                if self.active_trade:
                    try:
                        _sm_sid = self.active_trade["security_id"]
                        _sm_ld = self.fetch_ltp_concurrent([str(_sm_sid)])
                        _sm_ltp = _sm_ld.get(str(_sm_sid), 0)
                        if _sm_ltp and _sm_ltp > 0:
                            side_aware_monitor(self, _sm_ltp)
                    except Exception as _sme:
                        log.warning(f"side_aware_monitor error: {_sme}")
                    # No daily trade-count/position-count cap. New entries are governed by
                    # directional conviction, expected move, Dhan available margin and execution safety.
                ...
                _daily_loss_limit = getattr(config, 'DAILY_LOSS_LIMIT_RS', None) or (balance * config.DAILY_LOSS_LIMIT_PCT / 100)
                _daily_loss_limit = (getattr(self,'_last_balance',None) or balance) * config.DAILY_LOSS_LIMIT_PCT / 100
                if self.daily_pnl <= -abs(_daily_loss_limit):
                    log.info("Daily loss limit hit. Waiting for EOD.")
                    time.sleep(300)
                    continue

                # Entry cutoff — FIX 2026-08-10: read from config.ENTRY_CUTOFF ("HH:MM")
                # instead of hardcoded 14:45, which silently overrode config (=14:30).
                try:
                    _ec_h, _ec_m = (int(_x) for _x in str(getattr(config, "ENTRY_CUTOFF", "14:30")).split(":"))
                except Exception:
                    _ec_h, _ec_m = 14, 30
                if current_time >= ist_time(_ec_h, _ec_m):
                    log.info(f"Past entry cutoff ({_ec_h:02d}:{_ec_m:02d}). Waiting for EOD.")
                    time.sleep(60)
                    continue

                # No trade-count cap. Daily loss circuit breaker above is the only
                # account-level trading halt.
                ...
                long_result = self.scan_for_breakout() if orb_rescan.is_entry_allowed(self) else None  # R2
                short_result = self.scan_for_breakdown() if getattr(config, "ENABLE_SHORTS", False) else None
                _regime = getattr(self, "regime", getattr(self, "market_regime", "NORMAL"))
                _lc = long_result[0] if long_result else None
                _shc = short_result[0] if short_result else None
                _lscore = _lc.get("long_score") if _lc else None
                _sscore = _shc.get("short_score") if _shc else None
                # V8 entry-stage re-score: add actual setup/entry/opportunity evidence so
                # 80+ conviction is only possible after a live trigger is present.
                try:
                    from dual_scorer import score_candidate_dual as _v8_score
                    import indicators as _v8_ind
                    def _v8_rescore(_cand, _side, _result):
                        if not _cand or not _result: return None
                        _entry=float(_result[1] or _cand.get('ltp') or 0)
                        _target=float(_cand.get('_expected_target') or _result[1] or _entry)
                        _move=abs(_target-_entry)/_entry*100 if _entry>0 else 0
                        _df=None
                        try:
                            _oh=self.dhan.get_ohlc_intraday(str(int(_cand.get('security_id'))),'NSE_EQ','5')
                            if isinstance(_oh,dict) and _oh.get('close'):
                                _df=pd.DataFrame({_k:[float(_x) for _x in _v] for _k,_v in _oh.items() if _k in ('open','high','low','close','volume')})
                        except Exception: pass
                        _lead={str(x[0]).upper() for x in getattr(self,'_leading_sectors',[])}
                        _lag={str(x[0]).upper() for x in getattr(self,'_lagging_sectors',[])}
                        _sec=str(_cand.get('sector','')).upper()
                        L,S,_= _v8_score(gap_pct=_cand.get('gap_pct',0),rs=_cand.get('rs',0),rvol=_cand.get('rvol',0),df=_df,ltp=_entry,nifty_gap=self.nifty_data.get('gap',0),indicators_mod=_v8_ind,sector_leading=_sec in _lead,sector_against=_sec in _lag,setup_quality_L=15 if _side=='LONG' else 0,setup_quality_S=15 if _side=='SHORT' else 0,entry_quality_L=10 if _side=='LONG' else 0,entry_quality_S=10 if _side=='SHORT' else 0,expected_move_pct=_move,breakout_confirmed=_side=='LONG',breakdown_confirmed=_side=='SHORT',sudden_move=abs(float(_cand.get('gap_pct',0) or 0))>=2.0)
                        _cand['long_score']=L; _cand['short_score']=S; _cand['_expected_move_pct']=_move
                        return L if _side=='LONG' else S
                    if _lc and long_result: _lscore=_v8_rescore(_lc,'LONG',long_result)
                    if _shc and short_result: _sscore=_v8_rescore(_shc,'SHORT',short_result)
                except Exception as _v8e:
                    log.warning(f'V8 entry-stage rescore skipped: {_v8e}')
                # ==== FIX 2026-08-10: L=None/S=None at pick_side -> NO_TRADE every scan ====
                # Scores are written onto _prefilter_candidates in pre_open_scan(), but pool
                # reforms (post_orb / periodic rescan) rebuild self.candidates with fresh dicts
                # lacking the score, so _lc/_shc arrive with long_score=None. Two-stage repair:
                #   (A) backfill by security_id from the scored pre-open snapshot (fast path)
                #   (B) if still None (rescan-added stock never in snapshot), score INLINE.
                def _inline_score(_cand, _which):
                    try:
                        import indicators as _ind_i
                        from dual_scorer import score_candidate_dual as _sd_i
                        import pandas as _pd_i
                        _sid_i = str(int(_cand.get("security_id")))
                        _df_i = None
                        try:
                            _oh_i = self.dhan.get_ohlc_intraday(_sid_i, "NSE_EQ", "5")
                            if isinstance(_oh_i, dict) and _oh_i.get("close"):
                                _df_i = _pd_i.DataFrame({_k: [float(_x) for _x in _v]
                                    for _k, _v in _oh_i.items()
                                    if _k in ("open","high","low","close","volume")})
                        except Exception:
                            _df_i = None
                        _nif_i = self.nifty_data.get("gap", 0) if hasattr(self, "nifty_data") else 0.0
                        _lead_i = [x[0] for x in getattr(self, "_leading_sectors", [])] if hasattr(self, "_leading_sectors") else []
                        _sec_i = _cand.get("sector", "") or ""
                        _lead_ok = any(_sec_i.upper() in str(x).upper() for x in _lead_i)
                        _vs_ok = bool(_lead_i) and not _lead_ok
                        _L_i, _S_i, _ = _sd_i(
                            gap_pct=_cand.get("gap_pct", 0), rs=_cand.get("rs", 0),
                            rvol=_cand.get("rvol"), df=_df_i, ltp=_cand.get("ltp", 0),
                            nifty_gap=_nif_i, indicators_mod=_ind_i,
                            sector_leading=_lead_ok, sector_against=_vs_ok,
                            return_breakdown=True)
                        log.info(f"SCORE INLINE ({_which}): {_cand.get('ticker','?')} L={_L_i} S={_S_i}")
                        return _L_i, _S_i
                    except Exception as _ie:
                        log.warning(f"inline score failed {_cand.get('ticker','?')}: {_ie}")
                        return None, None
                try:
                ...
                # ==== end FIX 2026-08-10 (structure repaired 2026-08-11) ====
                # Unified side selection: NIFTY regime + sector context are biases, never hard blocks.
                _side, _why = "NO_TRADE", "scores unresolved"
                try:
                    _sec_boost_L = 0.0; _sec_boost_S = 0.0
                    _lead_names = {str(x[0]).upper() for x in getattr(self, '_leading_sectors', [])}
                    _lag_names = {str(x[0]).upper() for x in getattr(self, '_lagging_sectors', [])}
                    if _lc is not None:
                        _cs = str(_lc.get('sector','')).upper()
                        if _cs and _cs in _lead_names: _sec_boost_L += 6.0
                    if _shc is not None:
                        _cs = str(_shc.get('sector','')).upper()
                        if _cs and _cs in _lag_names: _sec_boost_S += 6.0
                    _side, _why = policy_pick_side(
                        _regime, _lscore, _sscore,
                        sector_boost_L=_sec_boost_L, sector_boost_S=_sec_boost_S
                    )
                except Exception as _pe:
                    log.error(f"SIDE SELECT ERROR: {_pe}")
                log.info(f"SIDE SELECT: {_side} | L={_lscore} S={_sscore} regime={_regime} | {_why}")

                # SHORT branch: 30s DOWN-confirm, then Stage-4 execution (stub until wired)
                if _side == "SHORT" and short_result:
                    s_cand, s_entry, s_r = short_result
                    s_sid = s_cand['security_id']
                    s_orb = self.orb_data.get(s_sid, {})
                    s_orb_low = s_orb.get('low', s_entry)
                    s_rank = next((idx for idx, c in enumerate(self.candidates) if c.get('security_id') == s_cand.get('security_id')), 5)
                    s_buffer = s_orb_low * (0.05 + s_rank * 0.03) / 100
                    s_confirmed = True
                    log.info(f"  Confirming BREAKDOWN for 30 seconds...")
                    for s_check in range(3):
                        time.sleep(10)
                        s_cl = self.get_live_price(s_sid)
                        if s_cl >= s_orb_low - s_buffer:
                            log.info(f"  ❌ Fakeout: {s_cand['ticker']} back to ₹{s_cl:.2f} (ORB low ₹{s_orb_low:.2f})")
                            s_confirmed = False
                            break
                        log.info(f"  ✓ Check {s_check+1}/3: ₹{s_cl:.2f} < ORB low ₹{s_orb_low:.2f}")
                    if not s_confirmed:
                        log.info(f"  Skipping {s_cand['ticker']} - failed 30s breakdown confirmation")
                        continue
                    s_ltp = self.get_live_price(s_sid)
                    log.info(f"✅ CONFIRMED BREAKDOWN: {s_cand['ticker']} @ ₹{s_ltp:.2f} (held 30s below ORB)")
                    # STAGE 4a: robust short placement via side_aware_entry (tested path)
                    _short_cand = {
                        "symbol": s_cand.get("ticker", "?"),
                        "security_id": s_cand.get("security_id", ""),
                        "entry_price": s_ltp,
                        "expected_target": s_cand.get("_expected_target"),
                        "expected_move_pct": s_cand.get("_expected_move_pct"),
                        "planned_sl": s_cand.get("_planned_sl"),
                    }
                    try:
                        _res = side_aware_entry(self, _regime, None, _sscore, None, _short_cand)
                        if _res:
                            self.trades_today = getattr(self, "trades_today", 0) + 1
                            log.info(f"✅ SHORT ENTRY placed: {s_cand.get('ticker','?')} @ ₹{s_ltp:.2f}")
                        else:
                            log.info(f"SHORT entry not placed (pick_side/guard declined) {s_cand.get('ticker','?')}")
                    except Exception as _see:
                        log.error(f"❌ SHORT entry failed: {_see}")
                    continue

                # LONG branch: existing robust path (unchanged)
                result = long_result
                if result:
                    ...
                    # PRE-FETCH: get live balance NOW so sizing is instant on confirm
                    _pre_balance = self.dhan.get_balance() or getattr(self, "_last_balance", balance)
                    self._last_balance = _pre_balance
                    log.info(f"  Pre-confirm balance: Rs.{_pre_balance:,.2f} (used for sizing)")
                    ...
                    _long_cand_entry = {
                        ...
                        "expected_target": candidate.get("_expected_target"),
                        "expected_move_pct": candidate.get("_expected_move_pct"),
                        "planned_sl": candidate.get("_planned_sl"),
                    }
                    _regime_long = getattr(self, "regime",
                        getattr(self, "market_regime", "NORMAL")) or "NORMAL"
                    _long_score_entry = float(candidate.get("long_score") or 0)
                    _res_long = side_aware_entry(self, _regime_long, _long_score_entry, None, _long_cand_entry, None)
            ...
            # PHASE 6: End of day
            # Close ALL positions at EOD — primary + overflow
            _all_eod = ([self.active_trade] if self.active_trade else []) + list(getattr(self,'active_trades_list',[]))
            for _eod_t in _all_eod:
                try:
                    _eod_sid = _eod_t.get('security_id','')
                    _eod_ld = self.fetch_ltp_concurrent([str(_eod_sid)])
                    _eod_ltp = _eod_ld.get(str(_eod_sid), 0)
                    if _eod_ltp > 0:
                        self.active_trade = _eod_t
                        side_aware_exit(self, _eod_ltp, 'MANDATORY_EXIT_EOD')
                        log.info(f'EOD exit: {_eod_t.get("symbol","?")} @ {_eod_ltp}')
                except Exception as _ee:
                    log.warning(f'EOD exit failed: {_ee}')
            self.active_trade = None
            self.active_trades_list = []
                    ...
                    if isinstance(_pos, list):
                        for _p in _pos:
                            if str(_p.get("securityId", "")) == str(sid):
                                _live = int(_p.get("netQty", 0) or 0)
                                break
                    _side = str(self.active_trade.get("side", "LONG")).upper()
                    _sign_ok = (_live > 0) if _side == "LONG" else (_live < 0)
                    if _live != 0 and _sign_ok:
                        _txn = "SELL" if _side == "LONG" else "BUY"
                        self.dhan.place_order(int(sid), min(qty, abs(_live)), 0, _txn, "MARKET")
                        log.info(f"Emergency exit placed side={_side} ({abs(_live)} sh)")
                    else:
                        log.info(f"Emergency exit SKIPPED - no live { _side.lower() } (netQty={_live})")
                except Exception:
                    pass
