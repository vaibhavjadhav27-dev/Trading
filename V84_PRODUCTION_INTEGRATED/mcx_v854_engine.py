...
class NativeMCXData:
    ...
    def resolve_contracts(self, prefixes: Dict[str, str], today: date) -> Dict[str, Contract]:
        ...
        for key, prefix in prefixes.items():
            ...
            for row in rows:
                ...
                if exch != "MCX":
                    continue
                if inst != "FUTCOM":  # Strict: only FUTCOM, no OPTCOM/OPTFUT
                    continue
    ...
    def intraday(self, security_id: str, instrument: str = "FUTCOM",
        ...
        payload = {
            ...
            "interval": interval,
            "oi": True,
            "fromDate": from_dt or today,
            "toDate": to_dt or today,
        }
        ...
        if r.status_code == 429:
            ...
            if _V12_MCX:
                _retry, _msg = should_retry_after_http(429)
                log.warning(f"Dhan data API 429: {_msg}")
    ...
    def marketfeed_ltp(self, security_ids: List[str]) -> Dict[str, float]:
        ...
        if r.status_code == 429:
            ...
            if _V12_MCX:
                _retry, _msg = should_retry_after_http(429)
                log.warning(f"Dhan quote API 429: {_msg}")
        ...
        if isinstance(raw, dict):
            for segment_key, segment_data in raw.items():
                if isinstance(segment_data, dict):
                    for sid, obj in segment_data.items():
...
def choose_signal(snapshot: Dict[str, Any], min_score=72.0, max_extension_atr=1.8):
    ...
    if len(candidates)>1 and candidates[0]["score"]-candidates[1]["score"] < 8:
        return None
    # V12: execution quality gate (if quote/spread/depth data available)
    best = candidates[0]
    if _V12_MCX and snapshot.get("quote_age_s") is not None:
        _eq_ok, _eq_reason = mcx_execution_quality_ok(
            float(snapshot.get("quote_age_s", 999)),
            float(snapshot.get("spread_ticks", 999)),
            float(snapshot.get("slippage_ticks", 0)),
            float(snapshot.get("depth_ratio", 0))
        )
        if not _eq_ok:
            log.info(f"  [V12_MCX] Execution quality REJECTED: {_eq_reason}")
            return None
    return best
